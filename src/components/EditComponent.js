...
const EditComponent = ({type, id, item}) => {
    ...
    const [itemTitle, setItemTitle] = useState(parse(item.title.rendered));

    useEffect(()=>{
        setItemTitle(parse(item.title.rendered))
    },[setItemTitle,item.title.rendered]);

    const itemTitleChange = (e) => {
            ...
            <DragDropContext onDragEnd={onDragEnd}>
                ...
                <div className="page-content-holder">
                    <header className="page-title-area mt-4 mb-3 d-lg-flex d-sm-block">
                        <h1 className="mb-0 mr-10">Edit {type} - {parse(item.title.rendered)}</h1>
                    </header>
                    <section className="page-content">
                        <Form className="mb-4">
                            <Form.Group className="mb-3" controlId="formTitle">
                                <Form.Label className="text-capitalize">{type} Title</Form.Label>
                                <Form.Control type="text" value={itemTitle} onChange={itemTitleChange}/>
                            </Form.Group>
